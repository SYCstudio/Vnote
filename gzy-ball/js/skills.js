class Skill
{
	...
	bindKey()
	{
		...
		this.delta = Math.ceil(this.cost * Math.pow(window.cacheBallSpeed, 2.9));
		console.log(`GZY 已加载技能：[${keyName}]${this.name}，冷却 ${this.cd}，消耗 ${this.delta}`);
		this.lastCastTime = 0;
	}
	...
	cast()
	{
		...
		console.log(`GZY 消耗了 ${this.delta} 积分发动了技能 ${this.name}！\n${this.desc}`);
...
class SkillW extends Skill
{
	constructor(main)
	{
		super(main,
			"呆鸡鬼步",
			"",
			"GZY 发动在美国校队时领悟的绝技，5 秒内让篮球跟着 GZY",
			15,
			20,
			"W");
...
class SkillE extends Skill
{
	constructor(main)
	{
		super(main,
			"鸡贼闪烁",
			"",
			"GZY 利用自己长期打摆被抓的经验，向当前方向闪烁一小段距离",
			0.2,
			1.25,
			"E");
	}
...
class SkillR extends Skill
{
	constructor(main)
	{
		super(main,
			"肥鸡之祝",
			"",
			"Oier 们对 GZY 施加祝福，其它技能的 CD 和消耗降低了",
			30,
			6,
			"R");
	...
	cast()
	{
		...
		this.casting = setInterval(() =>
		{
			...
			if (!isover)
			{
				...
				skillq.delta = Math.ceil(skillqdlt / 4);
				skillw.delta = Math.ceil(skillwdlt / 4);
				skille.delta = Math.ceil(skilledlt / 4);
			}
		}, 1000 / 60);
